package com.willian.usuario.interfaces;
...
public interface I_usuario extends CrudRepository<usuario, String> {
    ...
    @Query ("SELECT u FROM usuario u WHERE TIMESTAMPDIFF(YEAR, u.fecha_de_nacimiento, NOW())>=18 AND tipo_documento = 'TI'")
    List<usuario> cambiarTipoDocumento();

    @Query("SELECT u FROM usuario u WHERE  DATEDIFF(NOW(), u.fecha_ultima_actualizacion_contrasena) >= 0")
    List<usuario> actualizarContraseña(String actualizarContraseña);

    @Query("SELECT u FROM usuario u WHERE  DATEDIFF(NOW(), u.fecha_del_ultimo_inicio_sesion) >= 0")
